...
def functional_analysis(table, clustering_features=['style', 'syntax', 'structural', 'semantic'], dimensionality_reduction='off', clustering_method='k-means', melva_config=MELVA_DEFAULT_CONFIG):
	...
	kind_functions = {}
	# INFO: Alternative 1 = select groups of features and cluster according to them individually.  Then
	# INFO+ compute the average of the $r_i$, $s_i$, and $c_i$ scores, cf. Section "Step 1: reducing feature dimensionality"
	# INFO+ in the article on TOMATE.
	"""
	...
	"""
	# INFO: Alternative 2 = compute the clustering using all of the features, which might be more appropriate in the 
	# INFO+ case of Melva since it can find subspaces of features automatically.
	""" """
	xpath_table = [[cell['data-xpath'] if cell != PADDING_CELL else '' for cell in row] for row in table.elements]
	table.functions, confidence = cluster_vector_table(table.features, xpath_table, dimensionality_reduction, clustering_method)
	function_correction(table)
	""" """


def cluster_vector_table(instances, identifiers=None, dimensionality_reduction='off', clustering_method='k-means', melva_config=MELVA_DEFAULT_CONFIG):
	...
	elif clustering_method == 'melva':
		clusterer = MelvaClusterer(configuration=melva_config)
	with catch_warnings():
		simplefilter('ignore')
